...
@section('content')
    <div class="page-header">
        <!-- título da sessão -->
        <h1 class="title text-uppercase">Gerenciar: <b>Permissões</b></h1>
    </div>
    ...
    @if( isset($errors) && count($errors) > 0 )
        ...
        <form class="form" method="post" action="/painel/permissions/edt/{{$data->id}}">
            {{csrf_field()}}
            <div class="form-group">
                <label>Nome da permissão</label>
                <input type="text" name="name" placeholder="Ex: 'edita_paises'" value="{{ $data->name or old('name')  }}" class="form-control" >
            </div>
            <div class="form-group">
                <label>Descrição</label>
                <input type="text" name="label" value="{{ $data->label or old('label')  }}" class="form-control" >
            </div>
            ...
            <form class="form" method="post" action="/painel/permissions/del/{{$data->id}}">
                {{csrf_field()}}
                <div class="form-group">
                    <label>Nome da permissão</label>
                ...
                <div class="form-group">
                    <label>Descrição</label>
                    <input type="text" name="label" value="{{ $data->label or old('label')  }}" class="form-control" disabled>
                </div>
                <div class="form-group">
                ...
                <form class="form" method="post" action="/painel/permissions/add">
                    {{csrf_field()}}
                    <div class="form-group">
                        <label>Nome da permissão</label>
                        <input type="text" name="name" placeholder="Ex: 'edita_paises'" value="{{ $data->name or old('name')  }}" class="form-control" >
